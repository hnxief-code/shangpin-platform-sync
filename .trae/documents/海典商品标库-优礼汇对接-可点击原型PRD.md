...
- **详情页（双形态自适应展示）**：
  - **来源为“优礼汇”时**：
    ...
    - **审核字段清单**：
      - 基础信息（优礼汇侧需变更关注字段）：商品编码、商品名称、商品条码、规格、单位、产地、保质期、生产企业
      - 标库必填字段(默认赋值/不可编辑)：商品类型、批准文号、注册证书、是否散装中药、商品功效大类、剂型、商标、是否含特殊药品复方制剂
      - 类别信息：商品类别
      - 图片：商品主图(1张)、商品橱窗图(5张)
  - **来源为“非优礼汇”（来源平台展示为“-”）时**：
...
stateDiagram-v2
    ...
    待审核(PENDING) --> 入库失败(STOCK_IN_FAILED) : 运营点击[通过]但标库入库/更新事务失败
    入库失败(STOCK_IN_FAILED) --> 待审核(PENDING) : 运营手动点击[重试入库]
    待审核(PENDING) --> 通过待同步(APPROVED_WAIT_CALLBACK) : 运营点击[通过]且标库入库/更新成功
...
stateDiagram-v2
    ...
    待审核(PENDING) --> 已驳回(REJECTED) : 运营点击[驳回]
    待审核(PENDING) --> 入库失败(STOCK_IN_FAILED) : 运营点击[通过]但标库更新事务失败
    入库失败(STOCK_IN_FAILED) --> 待审核(PENDING) : 运营手动点击[重试入库]
    待审核(PENDING) --> 通过待同步(APPROVED_WAIT_CALLBACK) : 运营点击[通过]且标库更新成功
    通过待同步(APPROVED_WAIT_CALLBACK) --> 已完成同步(DONE) : 接口回传/同步成功
    通过待同步(APPROVED_WAIT_CALLBACK) --> 同步失败(CALLBACK_FAILED) : 接口回传/同步失败(超时/网络异常)
    同步失败(CALLBACK_FAILED) --> 通过待同步(APPROVED_WAIT_CALLBACK) : 运营手动点击[重试同步]
```
...
- **新品重复提交的状态不回退**：若该商品的新品审核记录已进入`已完成同步(DONE)`（或已生成平台商品编码），后续新品重复提交仅回传既有平台商品编码与“已审核”状态，不再将状态回退为`待审核(PENDING)`。
- **资料变更重复提交的记录策略**：若该商品存在已审核通过（DONE）的资料变更单据，后续再次提交资料变更时需新增一条审核记录（新增一行），不覆盖已完成的历史审核记录；若当前仍为待审核/同步中，则按最新推单覆盖该未完成记录。
- **标库8字段校验（标库层）**：运营点击“审核通过”后，写入标库前对8个核心字段进行一致性匹配。若命中已有标库记录，则按“更新标库资料”执行（Upsert-Update）；若未命中，则新增入库并生成平台商品编码（Upsert-Insert）。两种情况均视为入库成功，单据进入`通过待同步(APPROVED_WAIT_CALLBACK)`。
- **入库失败(STOCK_IN_FAILED)**：在执行标库表写入/更新的数据库事务阶段失败（如SQL异常、锁超时等），不得回传“审核通过”，需记录失败原因（error_code / error_msg），并允许运营点击“重试入库”幂等重放。
- **同步失败(CALLBACK_FAILED)**：仅针对已成功入库/更新，但跨系统通知优礼汇时遭遇网络异常的场景，允许人工介入重试同步。
