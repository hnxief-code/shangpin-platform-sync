...
- **详情页（双形态自适应展示）**：
  - **来源为“优礼汇”时**：
    ...
    - **审核字段清单及输入逻辑**：
      - **基础信息（文本输入/只读）**：商品名称、商品条码、规格、单位、产地、保质期、生产企业。文本输入框（Text），默认带入优礼汇数据，允许修改。
...
- **详情页**：
  ...
  - **特定变更字段限定**：当优礼汇平台中【商品主图、橱窗图、商品名称、商品条码、规格、单位、产地、保质期、生产企业】这些字段变更时，才生成资料变更审核记录，差异对比表仅罗列这些变更字段的具体值。其余字段（如箱规、税务字段等）不展示在资料变更详情页中。
  ...
  - **核心操作**：
    ...
    - **审核驳回**：同上。
  - **可编辑范围**：资料变更详情页中的“基础信息”仅用于展示（只读不可编辑）；仅允许修改“类别信息”，并通过勾选框决定上述变更字段是否更新到标库。
...
| 操作列按钮 | 列表页 | 点击“审核” | 仅待审核状态显示操作，路由跳转至对应的审核详情页；审核通过后操作列显示“--”，无需再打开查看。 |
...
- **重复提交覆盖（审核表层）**：优礼汇可对同一商品多次提交（新增/变更）。系统在写入审核表时按唯一键（来源平台+优礼汇侧商品ID+单据类型）进行 Upsert（覆盖写入）：若已存在同业务键的审核记录，则用最新推单内容覆盖原记录，并将状态重置为`待审核(PENDING)`，由运营重新审核。注意：优礼汇侧“商品ID”为表核心关联字段，无需在前端页面展示，运营仅关注“商品编码”等业务字段。
...
sequenceDiagram
    ...
    YLH->>HD: 1. 提交新品申请(携带唯一标识-商品ID)
    HD-->>HD: 2. 审核表写入(Upsert)：商品ID存在则覆盖原审核记录，状态统一置为待审核(PENDING)
    ...
    YLH->>HD: 1. 提交资料修改申请(携平台商品ID与变更字段)
    HD-->>HD: 2. 审核表写入(Upsert)：同一平台商品ID的变更单多次提交取最新，状态统一置为待审核(PENDING)
