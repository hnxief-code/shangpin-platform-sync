...
- **详情页（双形态自适应展示）**：
  - **来源为“优礼汇”时**：
    ...
    - **审核字段清单**：
      - 基础信息：商品编码、商品名称、税务编码、税务品名、商品分类、条形码、规格、单位、箱规、起批数
      - 详细信息：生产企业、服务商、产地、发票类型、税率、保质期(天)、标签、虚拟销量、备注
      - 多媒体与附件：商品主图、商品主视频、商品资质
  - **来源为“非优礼汇”（除优礼汇以外的如ERP等）时**：
...
- **详情页**：
  ...
  - **全量字段参考（优礼汇版）**：页面底部需折叠/平铺展示该商品的“当前完整商品信息 (参照优礼汇新增字段)”，以便运营人员结合上下文进行判断。
  - **核心操作**：
...
| 重试入库按钮 | 详情页右上角 | 点击 | 仅在“入库失败”状态出现，点击后重新触发“写入标库”的落库事务 |
| 重试同步按钮 | 详情页右上角 | 点击 | 仅在“同步失败”状态出现，点击后重新触发后端同步队列机制 |
...
```mermaid
stateDiagram-v2
    [*] --> 待审核(PENDING) : 优礼汇提交申请(唯一性校验通过)
    [*] --> 已驳回(REJECTED) : 优礼汇提交新增申请(商品编码已存在，系统自动驳回)
    
    待审核(PENDING) --> 已驳回(REJECTED) : 运营点击[驳回]
    待审核(PENDING) --> 已驳回(REJECTED) : 运营点击[通过]，但标库8字段唯一性校验失败
    
    待审核(PENDING) --> 通过待同步(APPROVED_WAIT_CALLBACK) : 运营点击[通过]且入库成功
    
    通过待同步(APPROVED_WAIT_CALLBACK) --> 已完成同步(DONE) : 接口回传/同步成功
    已完成同步(DONE) --> [*]
    
    通过待同步(APPROVED_WAIT_CALLBACK) --> 同步失败(CALLBACK_FAILED) : 接口回传/同步失败(超时/网络异常)
    同步失败(CALLBACK_FAILED) --> 通过待同步(APPROVED_WAIT_CALLBACK) : 运营手动点击[重试同步]
```

### 4.1 状态说明（研发实现口径）
- **系统自动驳回场景**：在新增申请推送时，系统执行第一重唯一性校验（商品编码已存在且为新增标识），单据仍会生成并落入审核表（供追溯），但状态直接置为`已驳回(REJECTED)`，并异步回传优礼汇。
- **标库校验失败驳回场景**：运营点击“审核通过”后，在写入标库前进行第二重唯一性校验（8个核心字段）。若发现冲突，则**不允许新增到标库中，该审核单据的状态被强制改写为`已驳回(REJECTED)`**，并记录失败原因，同步回传优礼汇。
- **重试同步(CALLBACK_FAILED)**：仅针对已成功入库，但在最后一步跨系统通知优礼汇时遭遇网络异常的场景，允许人工介入重试。

## 5. 数据交互流程图 (Sequence Diagram)
分为“新品申请”和“资料变更”两套核心数据流转流程。在此过程中，系统对**审核表**与**标库表**均增加了严格的【两重唯一性校验】与异常拦截机制。
...
sequenceDiagram
    ...
    HD-->>HD: 2. [第一重校验] 审核表唯一性校验(商品编码存在 且 为新增标识)
    alt 校验失败 (商品编码已存在)
        HD-->>HD: 3. 生成审核单据，但系统直接将其状态置为【已驳回】
        HD-->>YLH: 4. 异步回传驳回状态与原因
    else 校验通过
        HD-->>HD: 3. 生成待审核单据(状态: 待审核)
    end
    
    HD->>HD: 5. 运营人工核对信息并补充/编辑必填项后点击[审核通过]
    HD->>HD: 6. [补全入库] 根据默认值字典静默补全标库必填字段
    HD->>DB: 7. [第二重校验] 标库8字段唯一性校验
    note right of HD: 8字段: 商品类型,通用名,商品规格,生产企业,条形码,是否散装,批准文号,注册证号
    alt 8字段校验失败 (存在完全相同的记录)
        DB-->>HD: 8. 标库拒收入库
        HD-->>HD: 9. 单据状态强制改写为【已驳回】，记录失败原因
        HD-->>YLH: 10. 异步回传驳回状态
    else 8字段校验通过
        DB-->>DB: 8. 校验通过，新增数据入库，并自动生成【平台商品编码】
        note right of DB: 标库不扩展优礼汇字段，仅保存标库原有结构数据
        HD->>YLH: 9. 异步回传ID(【平台商品编码】)及审核状态
        YLH-->>YLH: 10. 优礼汇侧落库，关联该平台商品编码
    end
    end
    ...
    HD-->>HD: 2. [第一重校验] 资料变更无需系统自动驳回，直接进入人工审核池
...
3. **防重机制一（推单直接驳回校验）**：在优礼汇提交申请时，系统需立刻执行校验。若推单为“新增标识”，且审核表/标库中已存在该商品编码，**系统仍会生成单据，但必须触发熔断直接将其状态置为“已驳回”**，不进入待审核列表。资料变更申请则正常进入待审核状态由人工处理。
4. **防重机制二（入库8字段唯一校验）**：在运营点击“审核通过”写入标库前，必须执行标库层面的8个字段唯一性校验：`商品类型、通用名、商品规格、生产企业、条形码、是否散装、批准文号、注册证号`。若这8个字段与库中记录完全一致，**则拒收入库，并将该审核单的状态强行更新为“已驳回”**，返回失败结果。
...
### 7.2 优礼汇侧【特有】字段
现有海典大表单未包含，在渲染 `platform_id === 'YOU_LI_HUI'` 时需额外展示：
- **基础信息类**：税务编码（必填）、税务品名（必填）、箱规（必填）、起批数（必填）
- **详细信息类**：服务商、发票类型、税率、标签、虚拟销量
- **多媒体与附件**：商品主视频、商品资质（PDF/Word等附件文档）
- **其他**：优礼汇专属备注
